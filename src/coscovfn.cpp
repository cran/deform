...
double p3 = Rcpp::as<double>(pars[3]);
...
if (nz > 0) {
    for (int l=0; l < nz; l++) {
    z.col(l) = Rcpp::as<arma::mat>(X[l]) * Rcpp::as<arma::vec>(pars[4 + l]);
}
}
...
double ff1, ff2, ff3, ff4, ff5, ff6;
int nz = X.size();
...
double ee43, ee44, ee46, ee52, ee53, ee54, ee58;
double ee65, ee66, ee69, ee74, ee78;
...
for (int l=0; l < nz; l++) {
    Rcpp::List dzll(nz + 4);
    dp0[4 + l] = dp0z.slice(l);
    dp1[4 + l] = dp1z.slice(l);
    dp2[4 + l] = dp2z.slice(l);
    dp3[4 + l] = dp3z.slice(l);
    for (int m=l; m < nz; m++) dzll[4 + m] = dzlzm.slice(l * nz + m);
    dzl[l] = dzll;
}
...
out[2] = dp2;
out[3] = dp3;
for (int l=0; l < nz; l++) out[4 + l] = dzl[l];
